...
APPEND 'REPORT zprog_sflight_demo.' TO it_source.
...
APPEND '    IF mt_out IS INITIAL. MESSAGE ''No data'' TYPE ''S''. RETURN. ENDIF.' TO it_source.
...
APPEND '  DATA(lo) = NEW lcl_report( ). lo->fetch_data( ). lo->display_alv( ).' TO it_source.

INSERT REPORT 'ZPROG_SFLIGHT_DEMO' FROM it_source.
COMMIT WORK.
GENERATE REPORT 'ZPROG_SFLIGHT_DEMO'.
