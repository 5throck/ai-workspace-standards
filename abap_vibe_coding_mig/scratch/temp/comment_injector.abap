...
APPEND 'SELECTION-SCREEN BEGIN OF BLOCK b1 WITH FRAME TITLE TEXT-001.' TO it_source.
APPEND '  SELECTION-SCREEN COMMENT /1(31) TEXT-002.' TO it_source.
APPEND '  SELECT-OPTIONS: s_car  FOR sflight-carrid.' TO it_source.
APPEND '  SELECTION-SCREEN SKIP.' TO it_source.
APPEND '  SELECTION-SCREEN COMMENT /1(31) TEXT-003.' TO it_source.
APPEND '  SELECT-OPTIONS: s_con  FOR sflight-connid.' TO it_source.
APPEND '  SELECTION-SCREEN SKIP.' TO it_source.
APPEND '  SELECTION-SCREEN COMMENT /1(31) TEXT-004.' TO it_source.
APPEND '  SELECT-OPTIONS: s_dat  FOR sflight-fldate.' TO it_source.
...
APPEND '  DATA: lo_report TYPE REF TO lcl_report.' TO it_source.
APPEND '  CREATE OBJECT lo_report.' TO it_source.
APPEND '  lo_report->fetch_data( ).' TO it_source.
APPEND '  lo_report->display_alv( ).' TO it_source.

INSERT REPORT 'ZPROG_SFLIGHT_QUERY' FROM it_source.
COMMIT WORK.

" Update Text Pool with Comments
DATA: it_text TYPE TABLE OF textpool.
it_text = VALUE #( ( id = 'I' key = '001' entry = '항공편 조회' )
                   ( id = 'I' key = '002' entry = '▶ 항공사 선택 (Airline)' )
                   ( id = 'I' key = '003' entry = '▶ 항공편 번호 선택 (Connection)' )
                   ( id = 'I' key = '004' entry = '▶ 운항 날짜 선택 (Date)' ) ).
INSERT TEXTPOOL 'ZPROG_SFLIGHT_QUERY' FROM it_text LANGUAGE sy-langu.
COMMIT WORK.
