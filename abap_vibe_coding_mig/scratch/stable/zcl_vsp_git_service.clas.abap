...
CLASS zcl_vsp_git_service IMPLEMENTATION.
  ...
  METHOD handle_export.
    ...
    TRY.
        ...
        FIND REGEX '"packages"\s*:\s*\[([^\]]*)\]' IN lv_params SUBMATCHES DATA(lv_pkgs_str).
        IF sy-subrc = 0.
        ...
        FIND REGEX '"includeSubpackages"\s*:\s*(true|false)' IN lv_params SUBMATCHES DATA(lv_sub_flag).
        lv_include_sub = xsdbool( lv_sub_flag = 'true' OR lv_sub_flag IS INITIAL ).

        " Check for objects array (alternative to packages)
        FIND REGEX '"objects"\s*:\s*\[([^\]]*)\]' IN lv_params SUBMATCHES DATA(lv_objs_str).
