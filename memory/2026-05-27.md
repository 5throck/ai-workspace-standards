...
## Session Summary
fix: wrap git archive and tar in try/catch to suppress NativeCommandError
...
## Session Summary
fix: complete PS5.1 NativeCommandError series — new-project.ps1 versioned scaffold and binary pipe corruption

## Changes
- `scripts/new-project.ps1` — wrap `git archive` and `tar` in `try/catch`; switch from pipe to temp file for binary tar stream (#111, #112)
- `scripts/new-project.ps1` — add `exit 0` at end of `setup.ps1` to suppress false ⚠️ warning; improve error message for pre-v0.5.0 tags (#110)
- `.gitattributes` — add `eol=lf` to `*.md` rule to prevent CRLF corruption of SKILL.md frontmatter on Windows checkout (#111)
- `skills/*.md` — normalize CRLF to LF in all skill files (#111)
- GitHub — push `template-v0.5.0` tag so versioned scaffold is usable (#110)
- PR #109 — resolve merge conflict in `scripts/new-project.ps1`; fix CRLF frontmatter + missing `owner` field in 4 skill files

## Decisions
- **`try/catch` as the universal pattern**: All git native commands in scripts called via `&` from `new-project.ps1` must use `try { ... 2>&1 | Out-Null } catch { }` — `2>$null` is insufficient under inherited `ErrorActionPreference=Stop`. Applied to `git archive` and `tar` in `new-project.ps1`.
- **Temp file over pipe for binary streams**: PowerShell pipes convert binary to string, corrupting tar archives. `git archive --format=tar -o <tempfile>` + `tar -x -f <tempfile>` is the correct pattern for all future binary stream handling.
- **`*.md eol=lf` in .gitattributes**: YAML frontmatter parsers are sensitive to CRLF. Enforcing LF for all `.md` files at the git level prevents recurrence across all Windows contributors.
- **Version tag discipline**: `templates/VERSION` must be kept in sync with a `template-vX.Y.Z` git tag after each significant template update.

## Open Issues
- `template-v0.4.0` intentionally unsupported for versioned scaffold (predates `templates/common/` structure) — documented in error message

---

## Session Summary
docs: record full scaffold fix series in memory and changelog
