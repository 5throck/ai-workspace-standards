...
> **`Projects/` ディレクトリ（存在する場合）**: 一部のワークスペースのクローンでは、ローカルの
> 利便性のためだけに、サンプル/参照用プロジェクトをこのリポジトリの作業ツリー内
> `Projects/<name>/` に配置しています。各プロジェクトは `templates/co-*/` からスキャフォールド
> された、独自の `.git/`・リモート・履歴を持つ完全に独立したGitリポジトリです — ここに配置されて
> いるからといって、このリポジトリの一部になるわけではありません。これらはこのリポジトリ自体の
> ツールの対象外です。ワークスペースレベルのスクリプト・テスト・監査は `Projects/*` を読み書き
> したり再帰的に走査したりしてはいけません（例: ワークスペースルートでスコープ指定なしの
> `bun test` を実行すると、ネストされたすべてのプロジェクトのテストスイートまで巻き込まれます —
> スコープ指定のない再帰グロブではなく、`bun run test` → `tests/` のようにこのリポジトリ自体の
> ディレクトリにスコープを絞ったコマンドを使用してください）。
