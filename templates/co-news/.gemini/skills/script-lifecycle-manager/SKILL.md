...
> **L3 Projects**: When registering a new script in an L3 project's `scripts/SCRIPTS.md`, use `source: L2` (the origin layer) and `layer: L0+L1` (not `L0`). Scripts placed in `scripts/<variant>/` subdirectories should be registered in the sub-registry at `scripts/<variant>/SCRIPTS.md`, not the main registry. See §6.5 Script Lifecycle Management for Tier 3 filtering details.
