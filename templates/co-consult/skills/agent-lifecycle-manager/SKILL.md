...
description: >
  ...
  co-consult variant. Ensures agents follow the required frontmatter schema and are
  registered in AGENTS.md and docs/co-consult.context.md.
version: 1.0.0
status: active
owner: pm
prerequisites: none
...
4. Update `docs/co-consult.context.md § Agents` — add row to Agents table with `status: active`.
5. Update `agents/README.md` — add row to Available Agents table.

### Deprecating an Agent

1. In `docs/co-consult.context.md § Agents`, change agent `status` to `deprecated`.
...
- [ ] `docs/co-consult.context.md § Agents` matches `AGENTS.md`
