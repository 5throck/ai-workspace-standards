...
prerequisites: none
metadata:
  source: https://open.law.go.kr/LSO/main.do
  license: KOGL (`공공누리`)
  category: legal
  locale: ko-KR
  triggers:
...
7. **Save to Deliverables**: Store research findings in `deliverables/research/` per Output Destination Mapping in `docs/context.md`.
...
- None yet; if recurring lookups reveal stable target-specific parameter sets not covered below, capture them as `references/target-params-ko.json` following the pattern in `skills/k-dart/references/terms-ko.json`.
