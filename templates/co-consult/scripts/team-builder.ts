...
`);
  ...
  try {
    ...
    proposal = JSON.parse(raw) as TeamBuilderProposal;
  } catch (e) {
  ...
  const required = ["version", "timestamp", "teamName", "changes", "approvedBy", "approvedAt"];
  for (const field of required) {
    if (!(field in proposal)) {
      console.error(`${R}[ERROR] Proposal JSON missing required field: "${field}"${Z}`);
      process.exit(1);
    }
  }
  const changeFields = ["agentsToCreate", "agentsToConvert", "agentsToDelete", "skillsToCreate", "skillsToModify", "skillsToReassign", "workflowPhases"];
  for (const field of changeFields) {
    if (!Array.isArray(proposal.changes[field as keyof typeof proposal.changes])) {
      console.error(`${R}[ERROR] Proposal changes.${field} must be an array.${Z}`);
      process.exit(1);
    }
  }
