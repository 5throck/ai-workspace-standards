...
To mark an advisory resolved manually:

```bash
# Edit the file and change status
sed -i 's/^status: active/status: resolved/' security/YYYY-MM-DD-slug.md
```

To suppress a false positive permanently, delete the file.
