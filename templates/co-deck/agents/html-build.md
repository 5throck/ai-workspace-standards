...
- **ppt-engine.js inline completeness**: When inlining `ppt-engine.js` into a single-file HTML, verify the full file is included (currently 1065 lines, 3 logical sections). Confirm `function initPPT` exists in the output HTML — its absence causes a `ReferenceError` that silently breaks TOC, TTS, and thumbnail nav. (See: `memory/issues/toc-empty-drawer-missing-initppt.md`)
