...
| `gen-visual-images.ts` | 3.1.0 | active | Infrastructure-only dispatcher: reads slidedata.json, dynamically imports presentations/\<project\>/diagram-defs.ts, renders SVG → PNG to shared pool `presentations/assets/diagrams/`; auto-rewrites slidedata.json visualImage paths to `../assets/diagrams/` convention; **v3.1.0**: unified output to shared pool (was per-project `images/`); accepts both legacy and new path formats | `bun scripts/co-deck/gen-visual-images.ts --project presentations/<proj>` |
...
*Last Updated: 2026-06-24 — gen-visual-images.ts v3.1.0 (unified output to shared pool `presentations/assets/diagrams/`); previous: gen-slides-pdf.ts v1.7.0 (background image rendering with overlay)*
