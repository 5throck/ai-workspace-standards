...
| `lib/theme-builder.ts` | 0.1.0 | active | Deterministic HTML deck builder: `buildThemeDeck(options)` — loads theme package, validates compatibility, replaces INJECT markers, inlines ppt-engine.js | `import from './lib/theme-builder'` (library, not CLI) |
| `build-theme-deck.ts` | 0.1.0 | active | CLI wrapper for theme builder: reads lecture-profile.md YAML frontmatter + slidedata.json, generates `lecture_v1.html` | `bun scripts/co-deck/build-theme-deck.ts --project <path> [--slide-data <path>] [--output <path>] [--version vN]` |
...
| `build-theme-preview.ts` | 0.1.0 | planned | Build preview iframe from production renderer | `bun scripts/co-deck/build-theme-preview.ts` |
...
*Last Updated: 2026-07-02 — verify-new-theme.ts v1.0.0 (composite registration gate); scaffold-theme-style.ts v2.0.0 (--from derivation, minimally valid stubs); added tests for verify-new-theme and scaffold-theme-style*
