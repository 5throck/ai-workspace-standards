...
| `handbook/check-search.ts` | 2.0.0 | active | Check ④: Validates search-manifest.json against actual HTML files, and search-data.js staleness detection; **v2.0.0**: manifest-driven (replaces hardcoded DOCS array parsing) | (library — imported by validate-nav) |
| `handbook/check-a11y.ts` | 1.0.0 | active | L2 — Accessibility checks: missing alt on img, heading hierarchy (stack-based container reset), empty links, missing html lang | `bun scripts/co-deck/handbook/check-a11y.ts --docs-dir docs` |
| `handbook/check-i18n-parity.ts` | 1.0.0 | active | Check ⑤: i18n parity validation of the built handbook (KO/EN pair coverage) | `bun scripts/co-deck/handbook/check-i18n-parity.ts` |
| `handbook/check-spell.ts` | 1.0.0 | active | L3 — Spell check for English HTML files: 197 hardcoded common misspellings; strips script/style/pre/code/URLs before checking | `bun scripts/co-deck/handbook/check-spell.ts --docs-dir docs` |
| `handbook/check-lint.ts` | 1.1.0 | active | L4 — HTML lint: inline styles (with allowlist for CSS custom props, SVG, flexbox), inline event handlers (XSS risk), deprecated tags, duplicate IDs, empty headings | `bun scripts/co-deck/handbook/check-lint.ts --docs-dir docs` |
| `handbook/check-external-links.ts` | 1.2.0 | active | L5 — External link checker with redirect following (max 5 hops), HEAD→GET fallback (403/405/501), browser User-Agent headers, 5s timeout, 5-request concurrency, skips creativecommons.org/github.com/claude.ai/anthropic.com/whatismyipaddress.com/platform.deepseek.com | `bun scripts/co-deck/handbook/check-external-links.ts --docs-dir docs` |
| `handbook/build-search-index.ts` | 1.0.0 | active | Reads docs/search-manifest.json and generates docs/assets/search-data.js containing SEARCH_DATA.DOCS + SEARCH_DATA.LABELS | `bun scripts/co-deck/handbook/build-search-index.ts --docs-dir docs` |
| `handbook/extract-copycode.ts` | 1.0.0 | active | Extracts inline copyCode() from HTML files into shared docs/assets/copy-code.js; replaces with script tag reference | `bun scripts/co-deck/handbook/extract-copycode.ts --docs-dir docs` |
| `handbook/scaffold-handbook.ts` | 1.2.0 | active | Generate handbook project scaffold — copies templates+assets+scripts (incl. all 21 validation scripts: structure/nav/tables/a11y/spell/lint/external-links/search-index/copycode), creates package.json (npm scripts for every check), CI workflow (validate-handbook + check-authoring + check-a11y + check-spell + check-lint + build-search-index staleness gate), and README/LICENSE; **v1.2.0**: scaffold now ships the full validation toolkit so `validate-handbook --checks all` works out of the box | `bun scripts/co-deck/handbook/scaffold-handbook.ts --project <path> [--output handbook/] [--lang ko] [--title ...] [--description ...] [--repo owner/name] [--chapters "Ch1,Ch2"]` |
| `handbook/check-authoring.ts` | 1.2.0 | active | AUTHORING_GUIDELINES compliance checker — 12 checks (visual elements, copy buttons, sidebar nav, chapter-nav, flex min-width, mid-word strong, course overview, CSS variables, language pairs, no private-repo refs, footer structure, instructor guide); supports `--examples-dir` for regression fixture validation (footer structure check skipped in examples mode since fixtures aren't a real deployed handbook) | `bun scripts/co-deck/handbook/check-authoring.ts --project <path> [--lang ko] [--examples-dir <path>]` |
...
| `handbook/validate-handbook.ts` | 1.1.0 | active | Unified handbook validator — single entry point aggregating every read-only check: structure (check-structure), navigation (validate-nav 4 checks), tables (check-tables); optional: authoring + doctor + a11y + spell + lint via `--checks all`; exit code 1 on failure; **v1.1.0**: runSubprocess try/catch error handling | `bun scripts/co-deck/handbook/validate-handbook.ts --docs-dir docs [--checks structure,nav,tables\|all]` |
| `handbook/deploy-handbook.ts` | 1.1.0 | active | Deploys handbook to GitHub Pages — automates repo creation, visibility, GitHub Actions workflow generation, Pages activation, and verification | `bun scripts/co-deck/handbook/deploy-handbook.ts --project . --output handbook --repo owner/handbook-name --visibility public` |
...
| `handbook/handbook-sync-audit.ts` | 1.0.0 | active | Handbook sync audit — 3 modes: `content` (workspace docs/designs+adr reflection via keyword quorum, optional --since), `structure` (broken internal + cross-handbook links, orphan pages across all handbooks under a root), `parity` (h2/h3 id-set diff across ko/en/ja/es variants with exact missing sections); exit 1 on findings | `bun scripts/co-deck/handbook/handbook-sync-audit.ts <content\|structure\|parity\|all> [--handbook-dir docs] [--workspace ws] [--handbook-root dir] [--since YYYY-MM-DD]` |
...
*Last Updated: 2026-08-29 — added 6 new handbook validation scripts (check-a11y, check-spell, check-lint, check-external-links, build-search-index, extract-copycode); updated check-search.ts to manifest-based v2.0.0; updated validate-handbook.ts with try/catch and new checks (a11y, spell, lint); updated check-structure.ts language regex; updated site-search.js to load SEARCH_DATA from search-data.js; added copy-code.js to skill assets*
