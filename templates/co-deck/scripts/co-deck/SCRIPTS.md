...
| `handbook/check-authoring.ts` | 1.1.0 | active | AUTHORING_GUIDELINES compliance checker — 11 checks (visual elements, copy buttons, sidebar nav, chapter-nav, flex min-width, mid-word strong, course overview, CSS variables, language pairs, instructor guide, no private-repo refs); supports `--examples-dir` for regression fixture validation | `bun scripts/co-deck/handbook/check-authoring.ts --project <path> [--lang ko] [--examples-dir <path>]` |
