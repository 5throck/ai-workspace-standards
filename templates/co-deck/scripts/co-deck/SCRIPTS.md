...
| `gen-visual-images.ts` | 3.0.0 | active | Infrastructure-only dispatcher: reads slidedata.json, dynamically imports presentations/\<project\>/diagram-defs.ts, renders SVG → PNG per slide; project generators are fully isolated in diagram-defs.ts | `bun scripts/co-deck/gen-visual-images.ts --project presentations/<proj>` |
...
*Last Updated: 2026-06-22 — gen-visual-images.ts v3.0.0 (infrastructure-only dispatcher; dynamic import of presentations/\<project\>/diagram-defs.ts); new diagram-helpers.ts v1.0.0 (shared utilities + palettes); previous: measure-layout.ts v1.1.0*
