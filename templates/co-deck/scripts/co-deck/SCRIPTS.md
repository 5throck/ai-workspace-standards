...
| `handbook/check-lint.ts` | 1.0.0 | active | L4 — HTML lint: inline styles (with allowlist for CSS custom props, SVG, flexbox), inline event handlers (XSS risk with copyCode allowlist), deprecated tags, duplicate IDs, empty headings | `bun scripts/co-deck/handbook/check-lint.ts --docs-dir docs` |
| `handbook/check-external-links.ts` | 1.0.0 | active | L5 — External link checker: HTTP HEAD with 5s timeout, 5-request concurrency, skipscreativecommons.org/github.com/claude.ai/anthropic.com | `bun scripts/co-deck/handbook/check-external-links.ts --docs-dir docs` |
