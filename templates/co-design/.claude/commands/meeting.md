# Multi-Agent Design Meeting

Run an interactive meeting where design agents discuss design decisions and collaborate on solutions.

**Usage:**
```bash
/meeting "design topic to discuss" --agents pm,design-lead,ux-researcher,visual-designer,prototype-engineer
```

**Example:**
```bash
/meeting "Should we use Figma or Sketch for our design system?" --agents pm,design-lead,visual-designer
```

**Available Design Agents:**
- **pm**: Design Project Manager (meeting facilitator)
- **design-lead**: Design System & Visual Direction Lead
- **ux-researcher**: User Research & Usability Testing Specialist
- **visual-designer**: UI Execution & Visual Design Specialist
- **prototype-engineer**: Interactive Prototyping Specialist

See [AGENTS.md](../AGENTS.md) for complete agent roster.
