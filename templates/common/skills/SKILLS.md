...
| `k-dart` | 2.1.0 | active | strategy-analyst | 2026-09-02 | — | Unified DART OpenAPI skill — disclosure query, financial parsing, line-item extraction (scope: common, l2_propagate); KR-scoped - deployed only to KR-target projects, pruned otherwise (registry: `country_scoped_assets` in `docs/workspace-schema.json`) v2.1: corp_code fallback chain (30-day-TTL bulk-directory cache -> bulk retry -> web fallback with company.json cross-validation; CORPCODE endpoint is /api/corpCode.xml returning a STREAMING ZIP - the .zip variant is a block page; parser handles data-descriptor entries), disclosure presets, shareholder signal, source-document text, shared fetch gate |
