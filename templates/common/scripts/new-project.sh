...
NODE_MODULES_PATH="$WORKSPACE_ROOT/node_modules"

# Check if dependencies are installed
if [ ! -d "$NODE_MODULES_PATH" ] || [ ! -d "$NODE_MODULES_PATH/js-yaml" ]; then
    echo ""
    echo "[CHECK] Dependencies not found or incomplete. Installing..."
    if command -v bun >/dev/null 2>&1; then
        if (cd "$WORKSPACE_ROOT" && bun install); then
            echo "  [OK] Dependencies installed successfully"
        else
            echo "  [WARN] bun install had issues - continuing anyway, but script may fail"
        fi
    else
        echo "  [WARN] bun not found - skipping dependency installation"
        echo "   Script may fail if dependencies are not installed."
        echo "   Install Bun from https://bun.sh or run 'bun install' manually."
    fi
else
...
for agent in "${L1_ONLY_AGENTS[@]}"; do
  if [ -f "$PROJECT_DIR/$agent" ]; then
    ...
    echo "  🗑️  Excluded L1-only agent: $agent"
  fi
done
...
if bun "$WORKSPACE_ROOT/scripts/audit.ts" --skip-memory; then
