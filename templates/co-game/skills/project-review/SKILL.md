...
3. **Announce context**:
   ```
     Project type: [workspace-root | co-develop | co-game | co-design | co-work | co-security | custom]
...
This skill is used by the PM agent to perform comprehensive project reviews. It auto-detects the project type (workspace root, variant project) and available agent roster, then dispatches agents in parallel to cover all review domains. The skill produces a prioritized improvement plan.

## Execution Steps

1. **Detect Project Context** — scan agents, determine project type, announce context
2. **Generate Execution Plan** — map agents to domains, present plan table, await user approval
3. **Dispatch Agents in Parallel** — use Agent tool with `run_in_background: true` (Claude) or `/meeting --dialogue` (Gemini)
4. **Collect and Synthesize Results** — aggregate findings into Critical/High/Moderate/Low table
5. **Generate Action Items** — create prioritized action item table with owner and phase

## Output Format

Markdown report with 4 priority tables (Critical/High/Moderate/Low), Strengths section, and Action Items table. Saved to memory on request.

## Related Skills

- `meeting-facilitation` — used for Gemini CLI platform dispatch
- `simulate-project-creation` — validates scaffolding during review
