...
### Fixed
- **[2026-06-24]**: perf(hooks): add SYNC_ACTIVE guard to pre-push.ts audit invocation — eliminates duplicate full audit.ts run during /sync pipeline (audit was running 3×; now runs 2× with lifecycle-only in pre-commit); pre-push v1.2.2→v1.2.3; estimated ~10-15s pipeline speedup
