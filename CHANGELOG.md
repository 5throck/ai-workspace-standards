...
### Fixed
- **[2026-06-22]**: fix(scripts): replace external `diff` subprocess with a cross-platform LCS-based `lineDiffCounts()` in `upgrade-project.ts` (v1.1.0→v1.1.1) — `diff` is absent on the Windows PATH and previously crashed `diffSummary` with null stdout; the new LCS dp computes added/removed line counts in-process with no external dependency. `scripts/SCRIPTS.md` registry version synced to 1.1.1
