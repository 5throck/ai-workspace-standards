...
# -- Auto-install dependencies (prevent missing js-yaml errors) ----------------
...
if ((-not (Test-Path $NodeModulesPath)) -or (-not (Test-Path (Join-Path $NodeModulesPath "js-yaml")))) {
    Write-Host ""
    Write-Host "[CHECK] Dependencies not found or incomplete. Installing..." -ForegroundColor Cyan
    if (Get-Command "bun" -ErrorAction SilentlyContinue) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  [WARN] bun install had issues:" -ForegroundColor Yellow
            Write-Host $installResult
            Write-Host ""
            Write-Host "  Continuing anyway, but script may fail if critical deps are missing..." -ForegroundColor Yellow
        }
    } else {
        Write-Host "[WARN]  bun not found - skipping dependency installation" -ForegroundColor Yellow
        Write-Host "   Script may fail if dependencies are not installed." -ForegroundColor Yellow
        Write-Host "   Install Bun from https://bun.sh or run 'bun install' manually." -ForegroundColor Yellow
    }
} else {
...
# Use robocopy with /COPY:DT to copy only data and timestamps, NOT security permissions (ACLs)
# This prevents permission issues when deleting the project folder later
robocopy $CommonDir $ProjectDir /E /COPY:DT /NFL /NDL /NJH /NJS | Out-Null
...
$allMdFiles | ForEach-Object {
    ...
    if (Select-String -Path $srcFile -Pattern '^extends:' -Quiet) {
        ...
        try {
            ...
            $mergeResult = & bun "scripts/helpers/merge-frontmatter.ts" $destFile $skeletonAbsPath "L2" 2>&1
            if ($LASTEXITCODE -ne 0) {
...
# Skip on Windows (not needed, causes permission issues)
$isWindows = $true
try {
    $isWindows = [System.Environment]::OSVersion.Platform -eq "Win32NT"
} catch {
    # Default to Windows if check fails
}
...
& bun "$WorkspaceRoot/scripts/audit.ts" --skip-memory
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
