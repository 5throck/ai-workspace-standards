 ...
 * real variant content). Skills with no parseable version are skipped
 * (fail-closed reporting for those belongs to the VA-03/VA-07 family).
 */
export function collectCatalogEntries(
  templatesDir: string,
  exclude: Set<string>,
): { entries: CatalogEntry[]; divergentSkills: Set<string>; findings: RegistrySyncFinding[] } {
  const findings: RegistrySyncFinding[] = [];
  ...
  if (!existsSync(templatesDir)) return { entries: [], divergentSkills, findings };
  ...
  return { entries, divergentSkills, findings };
}
 ...
 * `collectCatalogEntries` already reported them as `catalog-divergent`.
 */
 ...
 * divergence is reported separately and the sync never picks a winner. Pure.
 */
...
export function collectWorkspaceRegistryFindings(root: string): RegistrySyncFinding[] {
  ...
  if (existsSync(rootRegistryPath)) {
    ...
    findings.push(...collectCatalogDrift('skills/SKILLS.md (Variant-Exclusive)', catalog, catalogScan.entries, catalogScan.divergentSkills));
  }
