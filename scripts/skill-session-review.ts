...
function parseSkillsUsed(memContent: string): SkillEvidence[] {
    // No `$` alternative in the lookahead — with /m, `$` matches at every
    // line end, truncating the lazy capture to a single line. The /m `^`
    // is still required to anchor at the section heading's line start.
    const sectionMatch = /^## Skills Used\s*\n([\s\S]*?)(?=\n## |\n---\n)/m.exec(memContent)
...
async function runStructuralChecks(skills: string[]): Promise<string[]> {
    ...
    for (const skill of skills) {
        ...
        // Dependency re-check per touched skill (output surfaces via its own report)
