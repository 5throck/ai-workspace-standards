 ...
 * variant templates. Eight fail-closed layers (see
 * docs/designs/2026-08-29-procedure-schema-design.md §5):
...
const PROCEDURE_ID_RE = /^co-[a-z0-9]+-[a-z0-9-]+$/;
...
/** Discover variant template directories (co-*) under <root>/templates. */
function discoverVariants(root: string): string[] {
  const templatesDir = join(root, 'templates');
  if (!existsSync(templatesDir)) return [];
  return readdirSync(templatesDir).filter(
    (name) => name.startsWith('co-') && statSync(join(templatesDir, name)).isDirectory(),
  );
}

function listProcedureDirs(variantDir: string): string[] {
...
function resolveAgentFile(root: string, variant: string, key: string): string | null {
  for (const base of [join(root, 'templates', variant), join(root, 'templates', 'common')]) {
    const p = join(base, 'agents', `${key}.md`);
...
function resolveSkillFile(root: string, variant: string, key: string): string | null {
  for (const base of [join(root, 'templates', variant), join(root, 'templates', 'common')]) {
    const p = join(base, 'skills', key, 'SKILL.md');
    if (existsSync(p)) return p;
  }
  return null;
}

function loadOutputTypes(variantDir: string): Map<string, OutputTypeEntry> {
  const map = new Map<string, OutputTypeEntry>();
  const p = join(variantDir, 'procedures', '_output-types.yaml');
...
function validateProcedure(proc: LoadedProcedure, outputTypes: Set<string>, root: string, issues: Issue[]): void {
  ...
  if (typeof data.procedure_id !== 'string' || !PROCEDURE_ID_RE.test(data.procedure_id))
    err('L2', `procedure_id must match ^co-[a-z0-9]+-[a-z0-9-]+$, got ${JSON.stringify(data.procedure_id)}`);
  if (data.variant !== variant) err('L8', `variant field "${data.variant}" does not match template directory "${variant}"`);
  ...
  // procedures/<dirName>/ must match procedure_id suffix.
  const expectedSuffix = data.procedure_id.slice(variant.length + 1);
...
export function validateAll(root: string, onlyVariant?: string): Issue[] {
  ...
  for (const variant of variants) {
    ...
    if (procDirs.length > 0 && outputTypeMap.size === 0) {
      issues.push({
        layer: 'L3',
        file: join(variantDir, 'procedures', '_output-types.yaml'),
    ...
    for (const dirName of procDirs) {
      const file = join(variantDir, 'procedures', dirName, 'schema.yaml');
      if (!existsSync(file)) {
