...
| `design-lint.ts` | L0 | 2.0.0 | active | v2.0.0 (2026-09-26, spec docs/designs/2026-09-26-design-lint-registries-design.md; tickets T-20260926-028..031): sub-check runner — all emitted paths POSIX-normalized (toPosix; registry-path string literals, finding/file fields, --json projectRoot) so output is byte-identical across Windows/macOS/Linux (Windows CI caught backslash emission; second site — the human-report markdown table's file:line cell — and third — the icons sub-check's relFile and token-source discovery — normalized in the same pass) — the v1.0.0 raw-value scan becomes the `token-usage` sub-check (behavior unchanged) and four registry-backed sub-checks join it: `components` (docs/design/components.registry.yaml shape + token bindings vs the project token source + governed raw-value scan via the unchanged detector), `patterns` (docs/design/patterns.registry.yaml shape + composes_from cross-refs + `patterns:` declarations in docs/*.md, `pattern-waiver:` records INFO), `icons` (docs/design/icon-vocabulary.yaml shape + UI-source `<Icon name>`/`icon=` reference set membership), `fonts` (--font-* stacks need >= 1 fallback + a terminating generic family from the closed set). New flags: `--check <name>` (repeatable), `--json` (per-check status pass/fail/skipped), `--schema` (prints the three shape-only registry skeletons), `[project-root]` positional (default cwd). A missing registry is a SKIP (exit-0 contribution), never an error — invoking the lint is the project opt-in; hand-rolled validators + js-yaml (no zod, zero new deps); deterministic, standalone (nothing wired into core audit/dev-sync). v1.0.0 CLI preserved: `[paths...]`/`--dir` scope token-usage exactly as before, `design-token-exempt: <reason>` suppression untouched | —| L0+L1 | —|
...
| `helpers/ticket-store.ts` | L0 | 1.3.1 | active | v1.3.1 (T-20260926-026c): TICKET_ID_PATTERN enforced at ticketPath — ids reach join(dir, id.yaml), so an unvalidated id was a path-escape class (the MCP server already enforced the shape). Prior: v1.3.0 (T-20260926-021, design docs/designs/2026-09-26-runner-lock-retry-enforcement-design.md D2): DEFAULT_ATTEMPTS_CAP — `failed -> waiting` beyond the cap (2) is refused with an escalation demand; `--force` remains the documented escape; the "one retry, then escalate" rule is now mechanical instead of prompt prose. Prior: MoveOptions.result written to the ticket on done transitions (T-20260912-023); id allocation scans both tickets/ and tickets/governance/ so a create can never mint a same-day id that shadows or is shadowed across directories (T-20260912-025) | —| L0 | —|
...
| `verify-platform-lifecycle.ts` | L0 | 1.6.0 | active | v1.6.0 (T-20260925-002 promotion, user-authorized early with the zero-WARN precondition verified): SOAK_MIRRORS emptied — .agents/.codex skill legs, the codex prompts mapping leg, and the Check G .agents lockstep leg all run at fail severity (.hermes preceded 2026-09-25). Prior: v1.5.0 (T-20260927-001): v1.5.0 (T-20260927-001): Check G gains the .agents lockstep leg — .agents/commands (L0-resident, T-20260925-003) was previously a recorded exclusion with NO gate, letting project-review.md drift 2.5 months; the new leg enforces existence (gateguard excluded) + content parity (commit-push-pr.md recorded adapted in AGENTS_COMMANDS_ADAPTED) against .claude/commands, WARN soak per ADR-0055 (joins the T-20260925-002 promotion scope); live drift synced and negative-tested. common-contract.json common_commands rows gain agents_source/codex_source/agents-parity (command mirrors were invisible to the contract). Prior: v1.4.0 (T-20260925-010 + T-20260925-008): Check G generalizes the hardcoded ['.claude','.gemini'] pair loop into the COMMANDS_SURFACES registry — every platform's commands surface is a governed leg or a RECORDED exclusion (.agents L0-resident, .hermes native /skill-name); import.meta.main guard + exports enable subprocess-tested coverage. .hermes/skills promotes out of SOAK_MIRRORS to fail severity (live hermes-agent v0.21.4 E2E green: project skill discovery, frontmatter tolerance, AGENTS.md entry; AGENTS.md 56,850 chars exceeds Hermes' 20,000-char context cap → truncation, pin context_file_max_chars on onboarding). Prior: v1.3.0 (ADR-0088 W2): `.hermes/skills` joins PLATFORM_MIRROR_DIRS consumers and SOAK_MIRRORS (net-new leg soaks WARN per ADR-0055). Prior: v1.2.0 Check E/H iterate the four mirrors; Check F becomes n-way version sync (.claude-gemini disagreement keeps fail, other shapes soak WARN); Check G gains the .codex/prompts mapping leg; .agents/commands excluded per Finding D (spec docs/designs/2026-09-25-verifier-platform-expansion-design.md sites 1a-1d, D1) | —| L0+L1 | —|
| `verify-readme-sync.ts` | L0 | 1.4.1 | active | v1.4.1 (T-20260926-026 batch hygiene): import.meta.main guard added — importing the module executed the full audit (same class as the agent-similarity-analyzer fix, T-20260926-020c). Prior: —| —| L0+L1 | —|
