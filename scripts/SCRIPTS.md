...
| `ticket.ts` | L0 | 1.2.1 | active | v1.2.1 (T-20260926-020e): doctor staleness scan covers BOTH ticket populations — the service dir and the manual governance dir (a stuck manual `running` ticket was previously invisible). `create --not-before`, `list --kind`, `list --ready`, `move done --result` (required, not bypassable by --force) | —| L0 | —|
