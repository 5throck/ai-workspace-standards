...
**Enforcement scope** — Phase A `Projects/<name>/` READMEs are produced **self-service** via `scripts/generate-l3-readme.ts` (and at scaffold time by `create-l3-scaffold.ts`); there is no CI gate, consistent with the L3 Design Gate exemption (L3 projects manage their own design workflow; both scripts were renamed from their `l2` names on 2026-08-15, exemption unchanged). Phase B `templates/co-*/` READMEs are what WS-08 hard-enforces. Both paths call the same renderer (`helpers/generate-variant.ts`), so a Phase A README is structurally identical to its Phase B counterpart the moment the variant is promoted.
