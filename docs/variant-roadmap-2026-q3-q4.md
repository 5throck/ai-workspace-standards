...
**Status**: Approved & Execution Ready — **Execution Notes**: then-9th variant `co-news` promoted 2026-08-10, `co-export` beta 2026-08-08, `co-game` promoted to stable v1.0.0 on 2026-08-12 (see §2 for status per variant).  
...
The AI Workspace platform provides standardized multi-agent development environments categorized into 11 domain-specific L2 variant templates. While the Phase A/B baseline infrastructure established governance compliance, schema validation (`validate-templates.ts`), and platform file symmetry, the 2026 Q3–Q4 roadmap elevates these 11 variants into enterprise-grade, domain-specialized execution engines.
...
The variant templates in this roadmap's scope map directly to industry-standard open-source benchmarks and frameworks, detailing the exact agent counts and specialized rosters specified in [docs/designs/variant-templates-advancement-design.md](designs/variant-templates-advancement-design.md). (`co-hr` and `co-abap` postdate this roadmap's scope and are tracked separately.)
...
| **`co-consult`** | `consulting` | **OpenBB Platform** & **McKinsey frameworks** | **10 agents**<br>(`pm`, `market-researcher`, `financial-analyst`, `strategy-consultant`, `data-analyst`, `esg-analyst`, `risk-analyst`, `deliverable-writer`, `industry-analyst`, `qa-reviewer`) | Financial report templates, basic analysis skills | Live DART Open API corporate disclosure parser (KR country profile), MECE logic tree auditor, Executive HTML dashboard compiler | Real-time regulatory filing ingestion with structured logic auditing |
...
| **`co-news`** | `collaboration` | **Reuters/Sedaily-style business journalism** | **7 agents**<br>(`pm`, `reporter`, `fact-checker`, `financial-analyst`, `legal-researcher`, `style-editor`, `visual-editor`) | Business/finance journalism (KR country profile included; Phase A prototype → official variant 2026-08-10, **beta**) | Real DART filing ingestion (KR profile) + financial narrative briefs, source-verification ledgers, AI-tell reduction, financial infographic SVG generation | Verifiable, evidence-led financial journalism with citation ledgers |
| **`co-export`** | `trade` | **WTO-framework customs compliance (KR country profile included)** | **10 agents**<br>(`pm`, `customs-duty-drawback-specialist`, `export-control-compliance-specialist`, `foreign-regulatory-intelligence-analyst`, `fta-origin-analyst`, `halal-certification-specialist`, `hs-classification-specialist`, `logistics-coordinator`, `market-entry-strategist`, `trade-documentation-specialist`) | Import/export trade compliance (official variant, **beta** 2026-08-08) | HS classification workflow, FTA origin determination, customs duty drawback, export control screening, foreign regulation monitoring | Standardized cross-border trade compliance workflows |
...
- **`co-consult` → OpenBB & McKinsey Frameworks (10 Agents)**: Features an explicit roster of 10 agents (`pm`, `market-researcher`, `financial-analyst`, `strategy-consultant`, `data-analyst`, `esg-analyst`, `risk-analyst`, `deliverable-writer`, `industry-analyst`, `qa-reviewer`). Modeled after OpenBB's financial ingestion pipeline and McKinsey structured reasoning. The `k-dart` skill (KR-scoped, not an agent) fetches and parses DART corporate disclosures (KR country profile), `strategy-consultant` and `qa-reviewer` run MECE logic tree checks, and `data-analyst` with `deliverable-writer` synthesize executive HTML dashboards validated by `dev-sync.ts` (v1.5.0).
