...
> 신규 사업 추진에 필요한 **규정·절차·지침·체크리스트 등 제반 절차를 소유·운영**하고, **이를 시스템으로 구현해 유지보수**하며, **그 체계 위에서 실제 딜 검토를 수행**한다.

세 축은 **동등하다**. 어느 하나가 다른 하나의 부속이 아니다.

| 축 | 내용 |
|---|---|
| **A. 거버넌스 소유·운영** | 규정·절차·지침·체크리스트·양식의 제정·개정·해석·배포·교육, 그리고 준수 점검·예외 승인·성과 측정까지의 상시 운영 |
...
| Agent | Tier | 역할 |
|---|---|---|
| **`_lead/process-governance-manager`** | High | **도메인 리드.** 규정·절차·지침·체크리스트 제정·개정 소유, 문서 5계층 관리, RAG 판정체계, 규정 변경의 시스템 반영 통제 |
...
| **주간** | 의존성 취약점 스캔, 감사 로그 검토, 용량 추이, staleness 리포트, 오탐 알림 검토, 미정의 용어 리포트, **준수 점검 리포트·만료 임박 waiver** | `security-engineer`, `dba`, `glossary-curator`, `compliance-officer` |
| **월간** | **복구 훈련**, 접근권한 재검토, 쿼리 성능 검토, **RAG 임계값 캘리브레이션**, 거버넌스 KPI 집계, 해석례 검수·공표 | `dba`, `security-engineer`, `process-governance-manager`, `compliance-officer` |
...
| **지침 (Guideline)** | 어떻게 하면 좋은가 | **권고** — 미준수 시 사유 설명 | 도메인 에이전트 | 도메인 리드 |
| **체크리스트 (Checklist)** | 무엇을 빠뜨리지 말아야 하는가 | 항목별 `mandatory: true\|false` | 도메인 에이전트 | 도메인 리드 |
| **양식 (Template)** | 어떤 형태로 산출하는가 | 권고 | docs-writer | 도메인 리드 |

> **규정과 지침을 섞지 않는 것이 핵심.** 전부 "의무"로 만들면 현장이 형식적으로 우회하고, 전부 "권고"로 만들면 아무도 안 지킨다.
...
**에이전트 산출물은 그 자체로 게이트를 통과시키지 못한다.** 3단 검토(§10.1-2)를 거치되 **최종 승인은 사람 검토자**(`analysis_task.reviewed_by`)다. AI 1·2차 검토가 통과시킨 것도 사람이 거부할 수 있으며, **그 반려 사유는 1·2차 검토 규칙 개선으로 되먹임**된다.
...
| 1 | 미팅 12건 + 종합 | ~12 | ~2 |
| 2 | 규정집·절차·지침·체크리스트·매뉴얼3·지역프로파일5·용어집5·전환계획·ADR 59 | ~35 | **~10** |
| 2.5 | Thin Slice | ~10 | ~2 |
...
| **M8** | 통합 검토 — 계획 최종 고도화 | pm(진행) + 전 리드 | M0~M7 통합, 상충 해소, **미해결 쟁점의 수용 리스크 확정** |
...
| 2.4 | 지침·체크리스트·양식 (문서 5계층 강제력 표기) | 각 도메인 에이전트 |
...
| 최종 보고서 | **Stage 5.10 시스템 파일럿 대조의 골든 참조** |
...
**5.1이 5.2~5.7의 선행 조건**이므로 직렬. 그 외 독립 모듈은 **병렬 진행 가능** — `.gitattributes` union merge(§10.1-1 D-4)로 공유 파이프라인 파일의 브랜치 충돌이 해소되었다. Wave당 1 PR은 **검토 단위로서 권장**이지 기술적 제약이 아니다.
...
| **R8** | 파일럿 산출물의 행방이 없다 | **시드/골든 참조로 보존**, 양식을 기계 적재 가능하게 | §10.6 |
...
### 14.3 거버넌스 운영
① 필수 체크리스트 미완 상태로 게이트 통과 시도 → **차단** ② waiver 발급 후 재시도 → 통과 + 기록 ③ **만료된 waiver로 시도 → 다시 차단** ④ 동일 조항 waiver 임계 누적 → **개정 트리거 발화** ⑤ 지침(권고) 미준수 → 차단이 아니라 **사유 입력 요구**(차단하면 계층 구분 미구현) ⑥ 해석례 등록 후 검색
...
**결론**: 축 C(딜 검토)가 파일럿 다수(002~004, 011~014)로 실행 중인 것은 맞지만, 이는 현재 AI 1차·2차 검토까지만 실질적으로 작동하고 있다는 뜻이며, 3단 검토의 마지막 단계인 "사람 최종 승인"은 저장 메커니즘이 있어도 한 번도 실제로 채워진 적이 없다. §0의 "검토 체계: 3단"이라는 확정 요약은 **설계·시스템 구현 목표로는 유효**하지만, **실무 가동 실적으로는 아직 검증되지 않았다**는 점을 이 부록으로 명시한다. co-newbiz 저장소에서 (a) `analysis_task` 행을 생성·갱신하는 API/UI 경로 구현, (b) human-final 승인이 실제로 최소 1건 기록될 때까지는 축 C의 "실제 딜 검토 수행" 주장을 AI 검토 단계로 한정해 서술할 것을 권고한다.
