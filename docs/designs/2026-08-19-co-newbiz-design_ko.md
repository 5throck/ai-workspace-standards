...
> **★ 병목은 AI 실행이 아니라 사람 검토다.** 스킬 88개는 빠르게 생성되지만 누군가는 읽고 승인해야 한다. 이 재구성이 나오자 O-9·O-14·O-15가 연쇄적으로 풀렸다.
...
> **★ 착수 조건 (3차 회의 D-3)**: **검토자 확보**가 선행되어야 한다. 이건 일정 리스크가 아니라 **전제 조건**이다 — 검토자 없이 시작하면 산출물만 쌓이고 게이트가 열리지 않는다. 권장은 시나리오 C(전담 1명 + 법무·재무 도메인 검토 분담 0.3). 겸업 검토자만 확보되면 캘린더가 3~4개월에서 **7~9개월로 늘어난다**(O-16).
...
| 결정 | 내용 | 반영 |
|---|---|---|
| **D-1** | O-7 해소 — AI ~190세션 / 사람 ~37검토일, 캘린더 3~4개월(시나리오 C) | §10.1-1 |
...
| # | 항목 | 결론 |
|---|---|---|
| **O-7** | 전체 일정·리소스 추정 | **해소** — §10.1-1. AI ~190세션 / 사람 ~37검토일 + 파일럿 3주, 캘린더 3~4개월(시나리오 C) |
...
| # | 쟁점 | 사유 | 재논의 |
|---|---|---|---|
| **O-16** | **검토자 확보 여부·할당률** | O-7 추정의 전제이자 최대 단일 변수. 미확보 시 캘린더가 3~4개월 → **7~9개월**로 늘어난다. §10.3에서 **Stage 0 착수 조건**으로 격상 | **Stage 0 착수 전** |
