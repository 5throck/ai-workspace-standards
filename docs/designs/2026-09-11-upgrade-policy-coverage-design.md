...
- **D10 — coverage gate wired into `audit.ts` v2.34.0**: new auto-activating check (same convention as the ADR-0060 skill-graph drift gate) spawns `check-upgrade-coverage.ts --strict` whenever the checker exists. At project (L2) context the checker self-skips (no `templates/` tree), so both L0 and L1 copies of the pair run the gate unconditionally. Placeholder-in-delivered-file, WS-07 contamination, and JSON_MERGE health are now enforced on every audit instead of manual runs.
