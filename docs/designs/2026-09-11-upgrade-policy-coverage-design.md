...
## 11. Addendum (2026-09-12): the `.env.sample` PRESERVE decision reversed — ENV_SAMPLE SYNC (upgrade-project v1.23.0)

- **What changed**: this design's D-set retained `.env.sample` in `PRESERVE_FILES` because scaffold-time country pruning strips `# >>> country-scoped:<CODE>` blocks and a wholesale re-sync would re-inject them. The 2026-09-12 fleet inspection showed the cost of that avoidance: template env-key evolution (`DATA_GO_KR_API_KEY`, `ECOS_API_KEY`, `KRX_API_KEY`) never reached already-scaffolded projects (co-export had the k-ecos/k-krx skills but not their keys), and the first wholesale-delivery draft of the new pass would have clobbered co-price's 16 project-only keys.
- **D11 — country-aware MERGE delivery** (`upgrade-project.ts` v1.23.0, shared `lib/env-sample.ts`): `.env.sample` leaves PRESERVE for a dedicated ENV_SAMPLE SYNC pass. Template content is delivered through `pruneCountryScopedEnvBlocks()` with the project's detected country applied (scaffold parity — region-neutral receives the all-blocks-stripped form), then `mergeEnvSample()` merges it over the project copy: template wins same-NAME keys (placeholder normalization), project-only keys / section dividers / inline notes / commented-out documentation keys are preserved verbatim under a marker section; idempotent. Scaffold pruning and upgrade delivery share one parser (`lib/env-sample.ts`) so the two cannot drift (`helpers/prune-country-scoped-assets.ts` v0.3.1 delegates to it).
- **Testing**: `tests/unit/env-sample.test.ts` (20 cases: marker grammar, unbalanced safety, real-fixture merge shapes for co-price/co-newbiz/co-export, idempotence); live dry-runs on Projects/co-price + co-newbiz; `verify-country-prune.ts` 4/4 after the helper refactor.
