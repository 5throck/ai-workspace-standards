...
3. (Part 3) **Resolved 2026-08-21, first real review completed.** The workspace ran its first end-to-end Context Commonization Review this session: promoted the "Git / PR Workflow" section into `docs/context.md` (PR #578/#579), then had to manually track down and remove 4 stale leftover duplicates (`co-abap`, `co-architect`, `co-consult`, `co-game`) that lingered because `checkVariantContextCommonization()` only compares variants against *each other* — once fewer than 2 variants still carried a promoted section, it stopped being flagged at all. That gap, not review-scheduling cadence, was the actual pain point observed. Fix shipped: `checkStalePromotedContent()` (`audit.ts` v2.15.0) compares every `docs/<variant>.context.md` section directly against `templates/common/docs/context.md`'s own sections, so a leftover post-promotion duplicate is caught regardless of how many other variants still have it. The original question (scheduled task vs. manually-remembered trigger) is deprioritized — at 10 variants the manual trigger plus this new detection is still adequate; revisit the scheduling question only if variant count grows enough to make even this insufficient.
...
- **Part 3 — done.** `docs/context.md` moved from `DOCS_MERGE_FILES` to `VARIANT_DOCS_SYNC` in `upgrade-project.ts` (1.8.1 → 1.9.0), with `VARIANT_DOCS_SYNC`'s source resolution generalized to `resolveTemplate()` (variant dir, then common) so it can reach `docs/context.md`'s common-only SSOT. `docs/context.md` (2.1 → 2.3) gained a "Context Commonization Review" subsection under its existing Lifecycle Management heading, pointing back to this Part. `checkVariantContextCommonization()` implemented in `scripts/audit.ts` (WARN-only, mirrors `checkVariantScriptDrift()`'s line-overlap heuristic, section-scoped, section-parsing extracted to shared `scripts/helpers/context-sections.ts`) — a real run across the 10 stable variants at time of writing found 9 flagged headings, several at 100% overlap across 6-7 variants.
- **Execution tooling — done.** `scripts/promote-context-section.ts` (new) executes a promotion decision: extracts the canonical section, inserts it into `docs/context.md`, bumps its version footer, removes the section from each specified variant's `docs/<variant>.context.md`, and — critically — always prints a per-variant line diff against the canonical text before writing anything. This last part is not optional polish: a dry-run promotion of the "Git / PR Workflow" heading (83-100% overlap across 6 variants) found `co-security` used a `/sync "security: ..."` commit-type convention where the other 5 used `/sync "feat: ..."` — a real, load-bearing difference a percentage-only report would have silently discarded. `skills/context-commonization-review/SKILL.md` (new) documents the full detect → decide → execute → propagate → verify procedure.
- **First real review completed, and a tool defect found by using it.** Promoted the "Scripts" section from 7/10 variants. The tool's section-removal regex stops at the next `#{2,3}` heading, so a nested `### Hybrid Scripting` subsection of the promoted `## Scripts` was left as an orphaned duplicate in all 7 variant files and never made it into `docs/context.md` either — a genuine content loss for `co-consult`/`co-export` (the only two of the 7 with no second copy elsewhere in the file). Fixed by hand (added the missing subsection to `docs/context.md`, removed the 7 orphans); `promote-context-section.ts` itself was not yet fixed for this nested-heading class of bug at the time — flagged for follow-up, not blocking since no other pending candidate had the same shape. **Fixed 2026-09-22** (`promote-context-section.ts` v1.1.0 + `helpers/context-sections.ts` v1.6.0 nesting-aware removal: a promoted `##` section carries its `###` children into `docs/context.md` and removes them from every variant file; fenced `#` lines never terminate; landed 2026-09-22 hardening batch). `checkStalePromotedContent()` (`audit.ts` 2.14.1 → 2.15.0) also added, resolving Open Question #3 below.
- **Structural standardization (new, `validate-templates.ts` Check WS-09).** A follow-up question — could `docs/<variant>.context.md` files be standardized altogether, not just individual duplicated sections? — led to a narrower, safer answer: content standardization was rejected (Skills/Agents tables are genuinely variant-specific; the co-security and leftover-duplicate cases above both showed forcing sameness on real content is destructive), but 7 of 8 collaboration-family variants (`co-abap` excluded as a structurally distinct SAP/ABAP domain) already share the same top-level heading *skeleton* — Stack → Agents → Skills → [Environment Setup] → Development Workflow → Guidelines → File Organization Policy → Domain Rules — just with domain-flavored heading text. WS-09 enforces presence + relative order of these slots via an alias/regex table, never an exact-heading whitelist, so domain-specific extra headings stay allowed. Full spec: `docs/governance/variant-contract.md` § Context.md Structure Standard.
