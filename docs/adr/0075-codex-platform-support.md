...
- **Cost**: three new mirror copies of skills/prompts (disk + drift surface) — absorbed by the existing idempotent sync pipeline; validator work stays pair-based in this effort (N-platform constant refactor deferred to a follow-up governance ticket); pointer-follow risk on Desktop App handled by the annex fallback.
