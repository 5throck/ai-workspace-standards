...
| **deprecated** | Being phased out | Add frontmatter `status: deprecated` warning; reassign skills within 30 days; set `removal_review:` (default: deprecation + 90 days) |
| **archived** | No longer used (formerly documented as `retired` — use `archived`; the audit accepts `retired` as a legacy alias with a warning) | Move to `agents/_archive/`; delete after 90 days |

> **Removal review (2026-09-21)**: a deprecated agent must carry a `removal_review:` date in its frontmatter. `agent-lifecycle-audit.ts` FAILS (Check 14) when the date passes without a review decision (delete via explicit user request, archive, or a recorded extension) and when a deprecated agent has no date at all. The quarterly roster review consumes due reviews. Skill `owner:` references must name real agents on the L0 authoring surface (Check 12), and `handoff_to`/`handoff_from` entries must point at live agents (Check 13).
