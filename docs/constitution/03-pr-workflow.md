...
- **Universal Design Gate (ADR-0074)**: every code change at any tier (L0–L3) must carry spec activity — a design doc under `docs/designs/` registered via `scripts/spec-register.ts` — enforced FATAL by `audit.ts --spec-check` in the `/sync` pipeline (step 3.9); trivial changes use the `--spec-exempt=E1..E5` escape hatch (AGENTS.md §5.1.1). Project registries (`docs/specs/registry.json`) are add-if-missing upgrade seeds (ADR-0073 Amendment 2).
- **LLM Work Routing (ADR-0078)**: substantive LLM-assisted work (code, documents, designs, tests, scripts) must reach `/sync` through the agent-team path — PM triage → Design Gate → specialist dispatch — before a PR can carry it; output pasted from an external LLM chat is input material, not a deliverable.
...
**Conflicted-PR recovery (ADR-0081 / T-20260918-002)**: when a merge conflict with `origin/main` does land on an open PR branch despite §3.3, resolve the conflicts in the working tree (`git add` each resolution) and conclude through the gates with `bun scripts/dev-sync.ts --conclude-merge --body-file <body> "<message>"` — the standard gate battery runs over the resolved tree and the two-parent merge commit is created through the normal SYNC_ACTIVE context (bare `git commit` stays blocked). dev-sync also warns at pre-flight, naming the diverged shared pipeline files, when `origin/main` has advanced past the branch's merge-base; `--require-current-main` escalates that warning to an abort. Both mechanisms are recovery rails — §3.3 remains the primary rule.
