...
#### 8.18 Context Commonization Review (ADR-0050)

**`templates/common/docs/context.md` is the SSOT for content genuinely shared by every variant. A variant's `docs/<variant>.context.md` adds only variant-specific content.** ADR-0050 established this one-directional inheritance rule for scripts (§8.12). This section extends it to context files. As the variant count grows, unrelated variants independently reach for the same wording. Left unmanaged, the duplication compounds project-by-project instead of being fixed once at the source.

- **Trigger**: review after scaffolding a new variant (the `create-variant` skill), and at minimum every 5 new variants or once per quarter, whichever comes first.
- **Detection**: `checkVariantContextCommonization()` in `scripts/audit.ts` (mirrors `checkVariantScriptDrift()`) flags `docs/<variant>.context.md` sections with high textual overlap across variants. The check is WARN-only. It is a first-pass heuristic that requires human judgment, not an auto-fix.
- **Decision (architect-owned)**: content shared by nearly all variants → promote into `templates/common/docs/context.md` via `scripts/promote-context-section.ts`; the version-footer sync in `upgrade-project.ts` then propagates it to every project automatically. Content shared by a subset → extract a shared skill, or a `docs/_common/` reference the affected variants opt into. Every project pays the cost of reading the shared file — irrelevant content there taxes variants that never needed it. Coincidental, likely-to-diverge similarity → leave alone. A high overlap percentage is a hint, not a verdict.
- **Diff before write**: `promote-context-section.ts` shows a per-variant diff before writing anything. Near-identical text can still carry a deliberate, load-bearing difference.
- See `skills/context-commonization-review/SKILL.md` for the full procedure, and [ADR-0050](../../docs/adr/0050-variant-script-inheritance-and-golden-reference-ssot.md) Part 3 for rationale, thresholds, and worked examples.
