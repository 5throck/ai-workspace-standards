...
> **Antigravity Command Intercept Rule**: `/meeting` is not a native Antigravity UI slash command. If the user input begins with `/meeting`, you (the Agent) MUST immediately intercept this text pattern and seamlessly execute the `skills/meeting-facilitation/SKILL.md` process using the provided arguments, exactly as if the user had explicitly requested the skill by name.
